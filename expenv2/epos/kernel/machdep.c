...
void sys_time_precise_impl(time_t *sec, unsigned int *msec)
{
    uint64_t total_ms;

    total_ms = (uint64_t)g_startup_time * 1000ULL
             + (uint64_t)g_timer_ticks * 1000ULL / HZ;

    if (sec != NULL)
        *sec = (time_t)(total_ms / 1000ULL);

    if (msec != NULL)
        *msec = (unsigned int)(total_ms % 1000ULL);
}
...
void syscall(struct context *ctx)
{
    ...
    switch(ctx->eax) {
    case SYSCALL_time_precise:
        {
    time_t *psec;
    unsigned int *pmsec;

    psec = *(time_t **)(ctx->esp + 4);
    pmsec = *(unsigned int **)(ctx->esp + 8);

    sys_time_precise_impl(psec, pmsec);
    ctx->eax = 0;
    break;
        }
    case SYSCALL_time:
